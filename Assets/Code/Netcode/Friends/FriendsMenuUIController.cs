...
using Suika.Scripts.Friends;
using Suika.Scripts.PlayerDataManagement;
using Unity.Properties;
using Unity.Services.CloudCode.GeneratedBindings.SuikaUGSCloud.Models; //Generated Player refrence
using UnityEngine;
...
namespace Suika.Scripts.Friends
{
    ...
    public class FriendsMenuUIController : MonoBehaviour
    {
        ...
        [SerializeField] FriendsMenuView m_FriendsMenuView;
        [SerializeField] FriendsClient m_FriendsClient;
        [SerializeField] FriendsManager m_FriendsManager;
        [SerializeField] RandomProfilePicturesSO m_PremadePortraits;
        
        [SerializeField] Sprite m_ActiveTabButtonSprite;
        [SerializeField] Sprite m_DisabledTabButtonSprite;

        PlayerDataManager m_PlayerDataManager;

        bool m_IsAllPlayersListInitialized = false;
        bool m_IsFriendsListInitialized = false;
        string m_DefaultSearchMessage = "Search players or enter player name";

        HashSet<Button> m_DisabledHeartButtons = new HashSet<Button>();

        bool IsAllPlayersTabActive => m_FriendsMenuView.AllPlayersListView.style.display == DisplayStyle.Flex;
        
        public event Action<Player> FriendAdded;
        public event Action<Player> FriendRemoved;
        public event Action<string> GiftingHeart;
        public event Action<string> AddFriendByTag;
        ...
        void InitializeDependencies()
        {
            ...
            if (!m_FriendsClient) m_FriendsClient = GetComponent<FriendsClient>();
            if (!m_FriendsMenuView) m_FriendsMenuView = GetComponent<FriendsMenuView>();
            m_FriendsMenuView.Initialize(m_PlayerDataManager.PlayerDataLocal, m_PlayerDataManager.PlayerEconomyDataLocal);
        }
        ...
        void HandlePlayerListUpdate(List<Player> players)
        {
        ...
        void HandleFriendsListUpdate(List<Player> friends)
        {
        ...
        void SetupAllPlayersListView(List<Player> players)
        {
        ...
        void SetupFriendsListView(List<Player> friends)
        {
        ...
        void SetupListViewBase(ListView listView, List<Player> items)
        {
        ...
        void BindPlayerItemUI(VisualElement element, Player player, bool isFriendsList)
        {
        ...
        void SetupPortraitBackground(VisualElement playerPortrait, ProfilePicture portrait)
        {
        ...
        void SetupListViewButtons(VisualElement element, Player player)
        {
            // Gift heart button
        ...
        void SetupFriendListViewButtons(VisualElement element, Player friend)
        {
            var giveHeartButton = element.Q<Button>("GiveHeartButton");
            if (giveHeartButton != null)
            {
        ...
        void BindPlayerLabels(VisualElement element, Player player)
        {
            // Bind name label
        ...
        void HandleGiftHeartButtonClicked(Button heartButton, Player player)
        {
        ...
        void HandleSearchValueChanged(ChangeEvent<string> evt)
        {
            ...
            else
            {
                ...
                else if (IsAllPlayersTabActive)
                {
                    SetupAddFriendByNameButtonEvent(evt.newValue.Trim());
                }
                else
                {
                    m_FriendsMenuView.DisableSearchGiftHeartButton();
        ...
        void UpdateListViewVisibility(List<Player> filteredList)
        {
        ...
        void SetupSearchGiftHeartButtonEvent(Player playerInfo)
        {
            ResetSearchActionButtonText();
        ...
        void SetupAddFriendByNameButtonEvent(string playerName)
        {
            m_FriendsMenuView.SearchGiftHeartButton.text = "Add Friend";
            m_FriendsMenuView.EnableSearchGiftHeartButton();
            m_FriendsMenuView.SearchGiftHeartButton.clickable = new Clickable(() => AddFriendByTag?.Invoke(playerName));
        }

        void ResetSearchActionButtonText()
        {
            if (m_PlayerDataManager?.PlayerDataLocal == null)
                return;

            m_FriendsMenuView.SearchGiftHeartButton.text = m_PlayerDataManager.PlayerDataLocal.GiftHearts + " Hearts";
        }
        ...
        void RemoveUIEventHandlers()
        {
            if (!m_FriendsMenuView || m_FriendsMenuView.AllPlayerTabButton == null)
            {
                return;
            }
        ...
        void RemoveManagerEventHandlers()
        {
