...
namespace Suika.Scripts.Core
{
    ...
    {
        [SerializeField] string m_UgsCheckUrl = "https://services.api.unity.com/";
        ...
        bool m_LastKnownOnlineState = false;

        const float m_CheckInterval = 10f;
        Coroutine m_PeriodicCheck;
        bool m_IsCheckingConnectivity;
        WaitForSeconds m_PeriodicCheckWait;
        const string k_StatusLogEmoji = "🚦";
        ...
        IEnumerator PeriodicOnlineCheck()
        {
            m_IsCheckingConnectivity = true;
            while (m_IsCheckingConnectivity)
            {
                yield return m_PeriodicCheckWait;
                Utilities.Logger.LogDemo("NetworkConnectivity: Checking network...");
        ...
        bool IsOnlineReachable()
        {
            return Application.internetReachability == NetworkReachability.ReachableViaLocalAreaNetwork;
        }

        IEnumerator CheckUGSAvailability()
        {
        ...
        public void HandleNetworkException(Exception ex)
        {
            ...
            Utilities.Logger.LogWarning($"Handling network error: {GetErrorDescription(status)}. Exception: {ex.Message}");
        ...
        ConnectivityStatus InterpretHttpError(long statusCode)
        {
        ...
        static string GetErrorDescription(ConnectivityStatus status)
        {
