...
namespace SuikaScripts
{
    public static class DropperSpawnSequenceService
    {
        ...
        public static byte GetTier(uint seed, int spawnIndex) {
            uint safeSeed = seed == 0 ?
                DefaultSeed :
                seed;
            var safeSpawnIndex = (uint)math.max(spawnIndex, 0);
        ...
        public static bool TryGetTierDefinition(DynamicBuffer<SuikaPrefabTierBuffer> tierBuffer, byte tier,
            out SuikaPrefabTierBuffer tierDefinition) {
            for (var i = 0; i < tierBuffer.Length; i++) {
        ...
        public static bool TryGetFirstSphereDefinition(DynamicBuffer<SuikaPrefabTierBuffer> tierBuffer,
            out SuikaPrefabTierBuffer tierDefinition) {
            for (var i = 0; i < tierBuffer.Length; i++) {
    ...
    public partial struct DropperSpawnSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            var suikaConfigEntity = SystemAPI.GetSingletonEntity<SuikaGameConfig>();

            var stressTestEnabled = false;
            ...
            if (!stressTestEnabled && totalSpawnCount > 0) totalSpawnCount = 1;

            // 3. Consume all requests on the main thread BEFORE scheduling any jobs.
            foreach (var entity in requests) ecb.RemoveComponent<DropperInitialSpawnRequest>(entity);
            ...
            if (totalSpawnCount > 0 &&
                ...
                ecb.AddComponent(spawnConfigEntity, new DropperSpawnBlocked());
                ecb.SetComponentEnabled<DropperPreviewReady>(spawnConfigEntity, false);
        ...
        public partial struct DropperStressTestJob : IJobEntity
        {
            ...
            public void Execute(Entity entity, [EntityIndexInQuery] int entityInQueryIndex,
                ref DropperStressTestConfig stressConfig) {
                if (stressConfig.Enabled == 0)
                    return;

                stressConfig.TimeSinceLastSpawnSeconds += DeltaTime;
                if (!(stressConfig.TimeSinceLastSpawnSeconds >= stressConfig.SpawnIntervalSeconds)) return;
        ...
        public struct DropperSpawnParallelJob : IJobParallelFor
        {
            ...
            public void Execute(int index) {
                ...
                if (!DropperSpawnSequenceService.TryGetTierDefinition(tierBuffer, tier, out var tierDefinition)) return;
                ...
                ECB.SetComponent(index, spawnedItem, LocalTransform.FromPositionRotationScale(
                    DropperPosition,
                    quaternion.identity,
                    shape == DropperSpawnSequenceService.SphereShape ? sphereScale : 1f));
                ...
                // Mark spawned item as awaiting first collision so spawning remains blocked
                ECB.AddComponent(index, spawnedItem, new DroppedAwaitCollision { OwnerConfigEntity = ConfigEntity });
            }
        }
    }
}