...
namespace SuikaScripts
{
    ...
    public class SuikaGameConfigData : ScriptableObject
    {
        ...
        public SuikaBurstConfig[] BurstBubbleConfigs = Array.Empty<SuikaBurstConfig>();
    }
    ...
    public class SuikaPrefabDefinition
    {
        ...
        [Tooltip("Shape type: 0 = Sphere, 1 = Cylinder")]
        [Range(0, 1)]
        ...
        [Tooltip("Score points awarded when this tier merges")]
        [Min(0)]
        public int ScoreValue;

        [Tooltip("If enabled, merging two items of this tier spawns the burst spheres instead of normal merge promotion.")]
        public bool BurstOnMerge;

        [Tooltip("Index into Burst Configurations. Used only when BurstOnMerge is enabled.")]
        [Min(0)]
        public int BurstBubbleConfigIndex;
    }

    [Serializable]
    public class SuikaBurstConfig
    {
        [Tooltip("Friendly label for identifying this burst configuration in the inspector.")]
        public string Name = "BurstBubble";

        [Tooltip("How many non-mergeable spheres this burst spawns.")]
        [Min(0)]
        ...
        [Tooltip("Multiplier applied to burst sphere size when spawned.")]
        [Min(0.01f)]
        public float SphereSize = 1f;

        [Tooltip("Radius around the merge point where burst spheres are spawned.")]
        [Min(0f)]
        public float Radius = 0.6f;

        [Tooltip("Seconds after spawn before burst spheres are forced into sleep behavior.")]
        [Min(0f)]
