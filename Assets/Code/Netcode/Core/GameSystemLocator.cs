...
namespace Suika.Scripts.Core
{
    ...
    {
        static GameSystemLocator s_Instance;
        readonly Dictionary<Type, object> m_Services = new();

        // Reset static state on subsystem registration so play-mode/domain-reload restarts do not
        // keep stale singleton references or service registrations alive between sessions.
        ...
        void Awake()
        {
            ...
            {
                Utilities.Logger.LogError("Service Locator Singleton already exists!");
                Destroy(gameObject);
            }
        }
        ...
        // Registration method
        static void Register(Type type, object service)
        {
        ...
        static bool IsServiceRegistered(Type type)
        {
        ...
        public static void UnregisterSelf<T>(T service) where T : class
        {
            ...
            if (s_Instance.m_Services.TryGetValue(type, out var registeredService))
            {
                ...
                {
                    Utilities.Logger.LogWarning($"Attempted unauthorized unregistration of service type {type.Name}.");
                }
            }
            else
            {
